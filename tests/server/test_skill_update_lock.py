...
async def test_backup_copy_failure_keeps_original_package_and_lock(client, service, monkeypatch):
    name = "backup-copy-lock"
    root = (await _add_skill(client, name, "Original"))["root_uri"]
    fs = service.viking_fs
    old_content = await _download(client, f"{root}/SKILL.md")
    old_index = await _indexed_record(client, root, 0)
    original_copy = fs._copy_directory_under_tree_locks

    async def copy_then_fail(*args, **kwargs):
        result = await original_copy(*args, **kwargs)
        if kwargs["old_uri"] == root and ".update-backup-" in kwargs["new_uri"]:
            await _assert_locked(fs, root)
            raise RuntimeError("injected backup copy failure")
        return result

    monkeypatch.setattr(fs, "_copy_directory_under_tree_locks", copy_then_fail)
    response = await client.put(
        f"/api/v1/skills/{name}",
        json={"data": _skill_md(name, "Replacement"), "wait": True},
    )
    assert response.status_code == 500, response.text
    assert "injected backup copy failure" in response.text
    assert await _download(client, f"{root}/SKILL.md") == old_content
    assert await _indexed_record(client, root, 0) == old_index


async def test_partial_content_cleanup_failure_restores_under_same_lock(
    client, service, monkeypatch
):
    name = "partial-cleanup-lock"
    root = (await _add_skill(client, name, "Original"))["root_uri"]
    fs = service.viking_fs
    old_content = await _download(client, f"{root}/SKILL.md")
    old_index = await _indexed_record(client, root, 0)
    ...
    async def remove_then_fail(uri, *args, **kwargs):
        nonlocal injected
        assert uri != root, "Replacing or restoring a Skill must never delete its root"
        result = await original_rm(uri, *args, **kwargs)
        if uri == f"{root}/SKILL.md" and not injected:
            injected = True
            await _assert_locked(fs, root)
            raise RuntimeError("injected partial content cleanup failure")
        return result

    monkeypatch.setattr(fs, "rm", remove_then_fail)
    response = await client.put(
        f"/api/v1/skills/{name}",
        json={"data": _skill_md(name, "Replacement"), "wait": True},
    )
