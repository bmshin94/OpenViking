...
@pytest.mark.asyncio
async def test_cancel_during_skill_lock_adoption_releases_acquired_lease(monkeypatch):
    ...
    async def resolve(*args, **kwargs):
        ...
        return SimpleNamespace(lock={"lease_ref": "skill"}, close=close)
    ...
    msg = SemanticMsg(uri="viking://agent/skills/demo", context_type="skill")
    worker = asyncio.create_task(
        SemanticProcessor()._resolve_skill_semantic_lock(msg, _ctx(), None)
    )
...
@pytest.mark.asyncio
@pytest.mark.parametrize("queue_kind", ["semantic", "embedding"])
async def test_cancel_waits_for_started_skill_retry_enqueue(monkeypatch, queue_kind):
    started = asyncio.Event()
    release = threading.Event()
    loop = asyncio.get_running_loop()
    enqueued = []
    ...
    def persist(msg):
        loop.call_soon_threadsafe(started.set)
        assert release.wait(3)
        enqueued.append(msg.id)
        return "queued"

    async def enqueue(msg):
        return await asyncio.to_thread(persist, msg)

    if queue_kind == "semantic":
        monkeypatch.setattr(
            "openviking.storage.queuefs.get_queue_manager",
            lambda: SimpleNamespace(
                SEMANTIC="Semantic", get_queue=lambda _name: SimpleNamespace(enqueue=enqueue)
            ),
        )
        msg = SemanticMsg(uri="viking://agent/skills/demo", context_type="skill")
        operation = SemanticProcessor()._reenqueue_semantic_msg(msg)
    else:
        monkeypatch.setattr(
            "openviking_cli.utils.config.get_openviking_config",
            lambda: _DummyConfig(_DummyEmbedder()),
        )
        msg = EmbeddingMsg("content", {"context_type": "skill"})
        handler = TextEmbeddingHandler(SimpleNamespace(enqueue_embedding_msg=enqueue))
        operation = handler._reenqueue_embedding_msg(msg)
