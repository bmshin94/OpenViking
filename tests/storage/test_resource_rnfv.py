...
def test_vector_snapshot_has_one_record_index() -> None:
    assert "record_ids_by_key" not in inspect.signature(VectorIndexSnapshot).parameters
...
def test_unchanged_content_with_replace_tags_produces_scalar_update() -> None:
    plan = build_diff_plan(_snapshot(tags=["env=test"], requested_tags=["team=search"]))
...
def test_modified_record_folds_tag_result_into_scalar_override() -> None:
    snapshot = _snapshot(tags=["env=test"], requested_tags=["team=search"], md5="old")
...
def test_modified_record_keeps_scalar_update_when_vectorization_is_disabled() -> None:
    original = _snapshot(tags=["env=test"], requested_tags=["team=search"], md5="old")
...
def test_body_compare_replans_scalar_action_after_content_result() -> None:
    snapshot = _snapshot(tags=["env=test"], requested_tags=["team=search"], md5="")
    plan = build_diff_plan(snapshot)
