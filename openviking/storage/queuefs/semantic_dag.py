...
class SemanticDagExecutor:
    ...
    async def _file_summary_task(self, parent_uri: str, file_path: str) -> None:
        ...
        if isinstance(entry, SemanticTreeEntry):
            need_vectorize = bool(
                semantic_succeeded and slot is not None and slot.operation == IndexOperation.UPSERT
            )
        if need_vectorize and not self._skip_vectorization:
            ...
            try:
                ...
                old_abstract = slot.abstract if slot is not None else ""
                summary_unchanged = bool(
                    self._semantic_plan is not None
                    and self._semantic_plan.file_vector_source
                    is FileVectorSource.SUMMARY_WHEN_AVAILABLE
                    and old_abstract
                    and str(summary_dict.get("summary") or "") == old_abstract
                )
                output_ready_upsert = bool(
                    slot is not None
                    and slot.operation == IndexOperation.UPSERT
                    and slot.trigger == SemanticOutputCondition.OUTPUT_READY
                )
                if (
                    summary_unchanged
                    and not output_ready_upsert
                ...
                elif summary_unchanged and not output_ready_upsert:
                    enqueued = False
                else:
                    if self._semantic_plan is not None:
                ...
                try:
                    ...
                    if self._semantic_plan is not None:
                        ...
                        def should_emit(level: int) -> bool:
                            ...
                            if slot is None or slot.operation is not IndexOperation.UPSERT:
                                return False
                            if slot.trigger is SemanticOutputCondition.OUTPUT_READY:
                                return True
                            if slot.trigger is not SemanticOutputCondition.OUTPUT_CHANGED:
                                return False
                            return (
                                write_result.abstract_body_changed
                                if level == 0
                                else write_result.overview_body_changed
                            )
