...
``ResourceDiffResult`` before building a ``ContextUpdatePlan``. The legacy
``DiffPlan`` helpers remain for callers that have not migrated yet.
"""
    ...
    *,
    ...
    if not require_vectors:
        ...
        plan = build_diff_plan(
            ...
            target_files=target_files,
            target_vectors=target_vectors,
            target_files_complete=rnfv.formal.complete,
            target_vectors_complete=True,
        )
    ...
    return ResourceDiffSnapshot(rnfv=rnfv, plan=plan)
